...
async def customer_workflow_stream(
    ...
    try:
        ...
        logger.info(f"Found chat thread: {chat_thread.id if chat_thread else 'None'} for session {session_id}")
        if not chat_thread:
            thread_data = CustomerChatThreadCreate(
                customer_id=current_customer.id, 
        ...
        # Get chat history
        messages = await get_chat_thread_messages(thread_id=chat_thread.id, limit=20, skip=0, current_customer=current_customer, db=db)

        # messages = await get_chat_thread_messages(db, chat_thread.id, limit=20)
        chat_history = [
            {"role": "user" if msg.speaker == "customer" else "assistant", "content": msg.content}
            for msg in messages  
        ]
        logger.info(f"Loaded {len(chat_history)} messages from chat thread {chat_thread.id} for customer {current_customer.id}: {chat_history}...")  # Log first 5 messages for brevity
...
def _save_conversation_in_background(
    ...
    try:
        ...
        with SessionLocal() as db:
            ...
            db.add(user_msg)
            db.flush()
            logger.info(f"User message added with sequence {user_msg.sequence_number} for interaction {interaction_id}")
            ...
            logger.info(f"AI message created with sequence {ai_msg.sequence_number} for interaction {interaction_id}")
            db.add(ai_msg)
            db.flush()
