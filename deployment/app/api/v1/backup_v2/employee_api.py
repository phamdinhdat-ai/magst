...
async def employee_workflow_query(
    ...
    try:
        ...
        try:
            ...
            result = await employee_workflow.process_with_load_balancing(
                query=request.query,
...
async def employee_workflow_stream(
    ...
    async def generate_stream():
        """Generator function for streaming response with load balancing support"""
        try:
            # Send initial metadata
            yield f"data: {json.dumps({'event': 'start', 'data': {'session_id': session_id, 'employee_id': current_employee.id, 'load_balancing_enabled': True}})}\n\n"
            
            # First try load balanced streaming
            try:
                logger.info(f"Starting load-balanced streaming for employee {current_employee.id}")
                
                # Check if load balancing is available and beneficial
                try:
                    ...
                    if load_balancer and load_balancer.is_active():
                        ...
                        # If system load is very high, try to forward to another node
                        if local_load.get('cpu_percent', 0) > 80 or local_load.get('memory_percent', 0) > 85:
                            yield f"data: {json.dumps({'event': 'system_info', 'data': {'message': 'High system load detected, attempting to forward request', 'load': local_load}})}\n\n"
                            
                            # Try to get result from load balancing instead of streaming
                            result = await employee_workflow.process_with_load_balancing(
                                query=query,
                                employee_id=str(current_employee.id),
                                employee_role=current_employee.role.value,
                            ...
                            if result.get("status") != "error":
                                # Send the result as streaming chunks to maintain SSE format
                                full_response = result.get("full_answer", result.get("agent_response", ""))
                                if full_response:
                                    ...
                                    for i in range(0, len(full_response), chunk_size):
                                        ...
                                        yield f"data: {json.dumps({'event': 'answer_chunk', 'data': chunk})}\n\n"
                                ...
                                final_event = {
                                    ...
                                    "data": {
                                        ...
                                        "suggested_questions": result.get("suggested_questions", []),
                                        "agents_used": result.get("agents_used", []),
                                        "processing_method": "load_balanced_forwarded"
                                    }
                                }
                                yield f"data: {json.dumps(final_event)}\n\n"
                                return
                except ImportError:
                    yield f"data: {json.dumps({'event': 'system_info', 'data': {'message': 'Load balancer not available, using direct streaming'}})}\n\n"
                ...
                async for event in employee_workflow.arun_streaming(
                    ...
                    # Add load balancing metadata to events
                    if isinstance(event, dict):
                        event.setdefault("metadata", {})["processing_method"] = "direct_streaming"
                    
                    # Stream each event as SSE
                    yield f"data: {json.dumps(event)}\n\n"
                    
            except Exception as stream_error:
                logger.warning(f"Streaming failed, attempting load balanced processing: {stream_error}")
                yield f"data: {json.dumps({'event': 'warning', 'data': {'message': 'Streaming failed, falling back to load balanced processing', 'error': str(stream_error)}})}\n\n"
                
                # If streaming fails, try load balanced processing
                result = await employee_workflow.process_with_load_balancing(
                    query=query,
                    employee_id=str(current_employee.id),
                    employee_role=current_employee.role.value,
                ...
                final_event = {
                    "event": "final_result",
                    "data": {
                        "full_final_answer": result.get("full_answer", result.get("agent_response", "")),
                        "suggested_questions": result.get("suggested_questions", []),
                        "agents_used": result.get("agents_used", []),
                        "processing_method": "load_balanced_fallback",
