# employee_retriever_mcp_client.py
import asyncio
import os
import json
import time
...
class EmployeeDocumentWatcher(FileSystemEventHandler):
    """File system watcher that triggers document uploads for a specific employee."""
    
    def __init__(self, retriever_tool: 'EmployeeRetrieverMCPClient'):
        self.tool = retriever_tool
        self.employee_id = retriever_tool.employee_id

    def _is_relevant_file(self, file_path_str: str) -> bool:
        """Check if file is relevant to this employee."""
    ...
    def on_created(self, event):
        if not event.is_directory and "_registry.json" not in event.src_path and self._is_relevant_file(event.src_path):
            logger.info(f"[Employee Watcher] New file detected for employee {self.employee_id}: {event.src_path}")
    ...
    def on_modified(self, event):
        if not event.is_directory and "_registry.json" not in event.src_path and self._is_relevant_file(event.src_path):
            logger.info(f"[Employee Watcher] File modified for employee {self.employee_id}: {event.src_path}")
            time.sleep(1)
            asyncio.create_task(self.tool._process_file_if_needed(Path(event.src_path)))
            
    def on_deleted(self, event):
        if not event.is_directory and "_registry.json" not in event.src_path and self._is_relevant_file(event.src_path):
            logger.info(f"[Employee Watcher] File deleted for employee {self.employee_id}: {event.src_path}")
            asyncio.create_task(self.tool._remove_from_registry(Path(event.src_path)))


class EmployeeRetrieverInput(BaseModel):
    """Input schema for employee document retrieval."""
    query: str = PydanticField(description="Search query for finding relevant employee documents")
...
class EmployeeRetrieverMCPClient(BaseAgentTool):
    """
    MCP client tool that connects to a remote MCP server for employee-specific document
    retrieval and ingestion. Watches a local directory for employee files and automatically
    uploads new/modified documents to the remote vector database with employee isolation.
    """
    
    name: str = "employee_retriever_mcp"
    description: str = "Retrieves employee-specific documents from remote vector database and manages document uploads"
    args_schema: type[BaseModel] = EmployeeRetrieverInput
    ...
    employee_id: str = PydanticField(description="Employee ID for document isolation")
    collection_name: str = PydanticField(description="Collection name for this employee")
    ...
    def __init__(self, 
                 mcp_server_url: str,
                 watch_directory: str,
                 employee_id: str = "nv_001",
                 **kwargs):
        """
        Initialize the employee MCP client tool.
        ...
        Args:
            ...
            employee_id: Employee ID for document isolation
        """
        # Sanitize employee ID for use in collection names
        ...
        super().__init__(
            ...
            name=f"employee_retriever_mcp_{safe_employee_id}",
            **kwargs
        )
    ...
    async def _initialize_all(self):
        ...
        logger.info(f"Initializing EmployeeRetrieverMCPClient for employee '{self.employee_id}'...")
        ...
        self._thread_pool = ThreadPoolExecutor(
            max_workers=4,
            thread_name_prefix=f"employee_mcp_{self.employee_id}"
        )
        ...
        if await self._test_server_connection():
            logger.info(f"MCP server connection successful for employee '{self.employee_id}'")
        else:
            logger.warning(f"MCP server connection failed for employee '{self.employee_id}' - some features may not work")
        ...
        logger.info(f"EmployeeRetrieverMCPClient initialized successfully for employee '{self.employee_id}'")
        ...
        try:
            async with sse_client(url=self.mcp_server_url) as streams:
                async with ClientSession(read_stream=streams[0], write_stream=streams[1]) as session:
                    ...
                    logger.debug(f"MCP server connection test successful for employee '{self.employee_id}'")
                    return True
        except Exception as e:
            logger.error(f"MCP server connection test failed for employee '{self.employee_id}': {e}")
            return False
    ...
    async def _load_document_registry(self):
        ...
        if self._registry_path.exists():
            try:
                ...
                logger.error(f"Failed to load registry for employee '{self.employee_id}': {e}. Starting fresh.")
    ...
    async def _save_document_registry(self):
        ...
        try:
            ...
            logger.debug(f"Registry saved with {len(self._document_registry)} entries for employee '{self.employee_id}'")
        except IOError as e:
            logger.error(f"Failed to save registry for employee '{self.employee_id}': {e}")
    ...
    def _load_and_split_file(self, file_path: Path) -> List[Document]:
        """Load and split a file into document chunks with employee-specific processing."""
        loader_map = {
            '.pdf': PyMuPDFLoader,
            '.csv': CSVLoader,
            '.json': JSONLoader,
        ...
        try:
            logger.info(f"Loading file: {file_path.name} with {loader_class.__name__} for employee '{self.employee_id}'")
            ...
            # Add source metadata with employee information
            for doc in split_docs:
                doc.metadata['source'] = file_path.name
                doc.metadata['file_path'] = str(file_path)
                doc.metadata['employee_id'] = self.employee_id
                doc.metadata['timestamp'] = time.time()
                
                # Add employee-specific metadata for better categorization
                if "contract" in file_path.name.lower():
                    doc.metadata['document_type'] = 'contract'
                elif "performance" in file_path.name.lower() or "review" in file_path.name.lower():
                    doc.metadata['document_type'] = 'performance_review'
                elif "training" in file_path.name.lower() or "certification" in file_path.name.lower():
                    doc.metadata['document_type'] = 'training'
                elif "salary" in file_path.name.lower() or "compensation" in file_path.name.lower():
                    doc.metadata['document_type'] = 'compensation'
                else:
                    doc.metadata['document_type'] = 'general'
            
            logger.info(f"Successfully processed {len(split_docs)} document chunks from {file_path.name} for employee '{self.employee_id}'")
            return split_docs
            
        except Exception as e:
            logger.error(f"Error loading file {file_path} for employee '{self.employee_id}': {e}")
            import traceback
            ...
            async with sse_client(url=self.mcp_server_url) as streams:
                async with ClientSession(read_stream=streams[0], write_stream=streams[1]) as session:
                    ...
                    if response and response.content:
                        ...
                        logger.info(f"Server response for employee '{self.employee_id}': {result_text}")
                        return "successfully" in result_text.lower()
                    else:
                        logger.error(f"No response from server for employee '{self.employee_id}'")
                        return False
                        
        except Exception as e:
            logger.error(f"Error uploading documents to server for employee '{self.employee_id}': {e}")
            return False
    ...
    async def _process_file_if_needed(self, file_path: Path):
        ...
        if not self._is_relevant_file(file_path):
            logger.debug(f"File {file_path.name} is not relevant to employee '{self.employee_id}', skipping")
            return
        ...
        if current_hash != stored_hash:
            logger.info(f"Change detected for '{file_path.name}' for employee '{self.employee_id}'. Processing...")
            ...
            if documents:
                ...
                if success:
                    ...
                    logger.info(f"Successfully processed and uploaded '{file_path.name}' for employee '{self.employee_id}'")
                else:
                    logger.error(f"Failed to upload '{file_path.name}' to server for employee '{self.employee_id}'")
            else:
                logger.warning(f"No documents extracted from '{file_path.name}' for employee '{self.employee_id}'")
    
    async def _scan_and_process_all_files(self):
        """Scan directory and process all relevant files for this employee."""
        logger.info(f"Scanning directory: {self.watch_directory} for employee '{self.employee_id}' files")
        
        current_files = set()
        for file_path in self.watch_directory.rglob('*'):
            logger.debug(f"Found file: {file_path}")
            if (file_path.is_file() and 
                not file_path.name.endswith('_registry.json') and
                self._is_relevant_file(file_path)):
                current_files.add(str(file_path))
                logger.debug(f"Processing file: {file_path} for employee '{self.employee_id}'")
    ...
    async def _remove_from_registry(self, file_path: Path):
        ...
        if path_str in self._document_registry:
            logger.info(f"Removing '{file_path.name}' from registry for employee '{self.employee_id}'")
    ...
    def _start_document_watcher(self):
        ...
        logger.info(f"Started document watcher on '{self.watch_directory}' for employee '{self.employee_id}'")
        ...
        try:
            async with sse_client(url=self.mcp_server_url) as streams:
                async with ClientSession(read_stream=streams[0], write_stream=streams[1]) as session:
                    ...
                    if response and response.content:
                        result_text = response.content[0].text
                        logger.info(f"Retrieved documents for employee '{self.employee_id}' with query: '{query[:50]}...'")
                        return result_text
                    else:
                        return f"No response from server for employee '{self.employee_id}'"
            ...
            error_msg = f"Error retrieving documents from server for employee '{self.employee_id}': {e}"
            logger.error(error_msg)
            return error_msg
    ...
    async def _arun(self, query: str, collection_name: str = None, max_results: int = 5) -> str:
        ...
        if not self._is_initialized:
            return f"Error: Employee retriever client for employee '{self.employee_id}' is not initialized."
        
        collection = collection_name or self.collection_name
        logger.info(f"Retrieving documents for employee '{self.employee_id}' with query: '{query}' from collection: '{collection}'")
        
        start_time = time.time()
        result = await self._retrieve_from_server(query, collection, max_results)
        retrieval_time = time.time() - start_time
        
        logger.info(f"Document retrieval completed for employee '{self.employee_id}' in {retrieval_time:.2f}s")
        return result
    ...
    async def upload_file(self, file_path: str, collection_name: str = None) -> str:
        ...
        if not self._is_relevant_file(path):
            return f"Error: File {file_path} is not relevant to employee '{self.employee_id}'"
        ...
        if success:
            ...
            if file_hash:
                ...
                await self._save_document_registry()
            return f"Successfully uploaded {len(documents)} document chunks from {file_path} for employee '{self.employee_id}'"
        else:
            return f"Failed to upload file: {file_path} for employee '{self.employee_id}'"
    ...
    async def upload_text_documents(self, texts: List[str], collection_name: str = None, source_name: str = "manual_upload") -> str:
        ...
        # Create Document objects with employee metadata
        documents = []
        ...
        success = await self._upload_documents_to_server(documents, collection)
        if success:
            return f"Successfully uploaded {len(documents)} text documents to collection '{collection}' for employee '{self.employee_id}'"
        else:
            return f"Failed to upload text documents to collection '{collection}' for employee '{self.employee_id}'"
    ...
    def cleanup(self):
        """Clean up resources."""
        logger.info(f"Cleaning up EmployeeRetrieverMCPClient resources for employee '{self.employee_id}'...")
        
        if self._observer and self._observer.is_alive():
            self._observer.stop()
            self._observer.join()
            logger.info(f"Document watcher stopped for employee '{self.employee_id}'")
        
        if self._thread_pool:
            self._thread_pool.shutdown(wait=False)
            logger.info(f"Thread pool shut down for employee '{self.employee_id}'")
    ...
    def __del__(self):
        ...
        try:
            ...
            logger.error(f"Error during cleanup for employee '{self.employee_id}': {e}")


# Example usage and factory functions
def create_employee_retriever_client(mcp_server_url: str, 
                                    watch_directory: str,
                                    employee_id: str) -> EmployeeRetrieverMCPClient:
    """
    Factory function to create an EmployeeRetrieverMCPClient instance.
    ...
    Returns:
        Configured EmployeeRetrieverMCPClient instance
    """
    return EmployeeRetrieverMCPClient(
        mcp_server_url=mcp_server_url,
...
if __name__ == "__main__":
    async def main():
        ...
        WATCH_DIR = "app/agents/retrievers/storages/employees"
        EMPLOYEE_ID = "nv_001"
        
        # Create the client
        client = create_employee_retriever_client(
        ...
        # Test retrieval
        query = "thông tin hợp đồng lao động"
        print(f"Query: {query}")
        ...
        # Test manual upload
        test_texts = [
            f"Employee {EMPLOYEE_ID} contract: Software Engineer position. Salary: 30,000,000 VND.",
            f"Employee {EMPLOYEE_ID} performance review Q1: Exceeds expectations in project delivery.",
        ]
        upload_result = await client.upload_text_documents(test_texts)
        print(f"Upload result: {upload_result}")
        
        # Test query again
        result2 = await client._arun("performance review Q1")
