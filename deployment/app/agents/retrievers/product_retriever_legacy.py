...
class DocumentWatcher(FileSystemEventHandler):
    ...
    def __init__(self, retriever_tool: 'ProductRetrieverTool'):
    ...
    def on_created(self, event):
        if not event.is_directory:
            ...
            time.sleep(1)
            self.tool._load_new_document(event.src_path)

    def on_modified(self, event):
        if not event.is_directory and "registry.json" not in event.src_path:
            logger.info(f"File modified: {event.src_path}")
            time.sleep(1)
            self.tool._reload_document(event.src_path)

class PerformanceCache:
    """High-performance cache for product retriever with TTL and memory management"""
...
class RetrievedProductDocument(BaseModel):
    """Enhanced product document with performance optimizations"""
    content: str
    source: str
    ...
    def get_processed_tokens(self) -> List[str]:
        """Get cached processed tokens for faster product text analysis"""
        if self._processed_tokens is None:
            # Enhanced tokenization for product/commercial terms
            self._processed_tokens = re.findall(r'\b\w+\b', self.content.lower())
        return self._processed_tokens
    
    def calculate_product_relevance(self, query_tokens: List[str]) -> float:
        """Calculate relevance with product-specific scoring""" 
        content_tokens = set(self.get_processed_tokens())
        query_token_set = set(query_tokens)
        
        # Product-specific term weights
        product_terms = {'product', 'item', 'brand', 'model', 'version', 'feature', 'specification', 'price', 'cost'}
        commercial_terms = {'buy', 'purchase', 'order', 'sale', 'discount', 'offer', 'deal', 'shipping', 'delivery'}
        quality_terms = {'quality', 'rating', 'review', 'performance', 'durability', 'warranty', 'guarantee'}
        
        # Calculate weighted intersection
        intersection_score = 0.0
        for token in query_token_set & content_tokens:
            if token in product_terms:
                intersection_score += 2.0  # Higher weight for product terms
            elif token in commercial_terms:
                intersection_score += 1.5  # Medium weight for commercial terms
            elif token in quality_terms:
                intersection_score += 1.5  # Medium weight for quality terms
            else:
...
class ProductRetrieverTool(BaseAgentTool):
    """Enhanced product retriever with performance optimizations"""
    name: str = "product_retriever"
    description: str = "Retrieves and reranks product information from a self-updating document knowledge base."
    ...
    _reranker_model: HuggingFaceCrossEncoder = PrivateAttr(default=None)
    _final_retriever: ContextualCompressionRetriever = PrivateAttr(default=None)
    _reranker_device: str = PrivateAttr(default='cpu') # <-- NEW: Default to CPU
    ...
    def __init__(self, watch_directory: str, collection_name: str = "product_docs",  reranker_device: str = 'cuda',**kwargs):
        ...
        self._thread_pool = ThreadPoolExecutor(max_workers=4, thread_name_prefix="product")
        self._batch_queue = []
    ...
    def _sanitize_collection_name(self, name: str) -> str:
        """Ensures collection name is valid for ChromaDB."""
        sanitized = re.sub(r'[^a-zA-Z0-9_.-]', '_', name)
        return sanitized[:63]

    def _initialize_all(self):
        """Orchestrates the full initialization process."""
        logger.info(f"Initializing ProductRetrieverTool for collection '{self.collection_name}'...")
        ...
        self._thread_pool = ThreadPoolExecutor(max_workers=4, thread_name_prefix="product_retriever")
        ...
        logger.info("ProductRetrieverTool initialized successfully.")
    ...
    def _registry_path(self) -> Path:
        """Returns the path to the registry file for this collection."""
    ...
    def _run(self, query: str) -> str:
        """Synchronously retrieves and reranks documents for the product."""
        try:
    ...
    async def _arun(self, query: str) -> str:
        """Asynchronously retrieves and reranks documents for the product with GPU management."""
        if not self._final_retriever:
    ...
    def unload_reranker_model(self):
        """Moves the reranker model to CPU and clears GPU memory."""
        if self._reranker_model is None and self._final_retriever is None:
            logger.info("No reranker model is currently loaded in memory.")
            return

        logger.info("Unloading reranker model from GPU...")
        
        # Access the underlying sentence-transformers model and move it to CPU
        try:
            # For HuggingFaceCrossEncoder, the actual model is in the '.model' attribute
            if self._reranker_model and hasattr(self._reranker_model, 'model'):
                 self._reranker_model.model.to('cpu')
                 logger.info("Model tensors moved to CPU.")
        except Exception as e:
            logger.error(f"Could not move model to CPU: {e}")

        # Delete the Python objects that hold references
        del self._reranker_model
        del self._final_retriever
        
        self._reranker_model = None
        self._final_retriever = None

        # Force garbage collection and empty the CUDA cache
        gc.collect()
        if torch.cuda.is_available():
            torch.cuda.empty_cache()
            logger.info("CUDA cache cleared. GPU memory should be released.")
    def _load_document_registry(self):
        """Loads the document registry from a file."""
        if self._registry_path.exists():
            try:
                with open(self._registry_path, 'r', encoding='utf-8') as f:
                    self._document_registry = json.load(f)
                logger.info(f"Loaded {len(self._document_registry)} entries from registry: {self._registry_path}")
            except (json.JSONDecodeError, IOError) as e:
                logger.error(f"Failed to load document registry: {e}. Starting fresh.")
                self._document_registry = {}
        else:
            logger.info("No document registry found. A new one will be created.")



    def _save_document_registry(self):
        """Saves the current document registry to a file."""
        try:
            with open(self._registry_path, 'w', encoding='utf-8') as f:
                json.dump(self._document_registry, f, indent=2)
            logger.info(f"Document registry saved with {len(self._document_registry)} entries.")
        except IOError as e:
            logger.error(f"Failed to save document registry: {e}")

    def _scan_and_process_all_files(self):
        """Scans the watch directory and processes all supported files."""
        logger.info(f"Scanning directory for all supported files: {self.watch_directory}")
    ...
    def _process_file_if_needed(self, file_path: Path):
        """Checks a file against the registry and loads/reloads it if necessary."""
        file_path_str = str(file_path)
        
        # Skip registry files
        if "_registry.json" in file_path_str:
            return
        ...
        stored_hash = self._document_registry.get(file_path_str)

        if not stored_hash:
            logger.info(f"New file detected: '{file_path.name}'. Processing...")
            self._load_new_document(file_path_str)
            return

        if current_hash != stored_hash:
            logger.info(f"Change detected in '{file_path.name}'. Processing...")
            self._reload_document(file_path_str)
    ...
    def _load_new_document(self, file_path: str):
        """Loads a new document, checks for duplicates, and adds to the store."""
        path = Path(file_path)
        path_str = str(path)
        
        if path_str in self._document_registry:
            logger.warning(f"File '{path.name}' is already in the registry. Use reload_document to update.")
            return
        
        self._reload_document(path_str)

    def _reload_document(self, file_path: str):
        """Deletes old versions and loads the new version of a document."""
        path = Path(file_path)
        path_str = str(path)

        # 1. Delete existing documents from this source
        self._delete_documents_by_source(path.name)
        logger.info(f"Reloading document: {path.name}") 
        ...
        if documents:
            ...
            if new_hash:
                self._document_registry[path_str] = new_hash
                self._save_document_registry()
                logger.info(f"Successfully reloaded and registered '{path.name}'.")
    ...
    def _add_documents_to_store(self, docs: List[Document]):
        """Adds documents to the vector store and updates the BM25 retriever."""
        if not docs:
            return
    ...
    def _delete_documents_by_source(self, source_filename: str):
        """Deletes all document chunks associated with a source filename."""
        try:
            # ChromaDB uses 'source' in metadata for filtering
            ...
            if existing_ids:
                ...
                logger.info(f"No existing documents found for source '{source_filename}'. Nothing to delete.")
        except Exception as e:
            logger.error(f"Failed to delete documents for source '{source_filename}': {e}")
            
    def _update_bm25_retriever(self, force: bool = False):
        """Re-initializes the BM25 retriever with throttling and caching."""
        ...
        try:
            ...
            if cached_docs:
                ...
                self._bm25_retriever = None
                logger.warning("No documents in vector store. BM25 retriever is disabled.")
    ...
    def retrieve_documents(self, query: str, use_cache: bool = True) -> List[str]:
        """Enhanced hybrid search with caching, parallel processing, and product-specific scoring."""
        if not self._is_initialized:
            return ["Error: Product retriever is not initialized."]
        ...
        for doc, score in vector_results:
            product_doc = RetrievedProductDocument(
                content=doc.page_content,
                source=doc.metadata.get("source", "unknown"),
                relevance_score=score
            )
            
            content_hash = product_doc.get_content_hash()
            if content_hash not in seen_hashes:
                seen_hashes.add(content_hash)
                hybrid_results[doc.page_content] = product_doc
        ...
        # Apply product-specific relevance scoring
        query_tokens = query.lower().split()
        for doc in hybrid_results.values():
            doc.calculate_product_relevance(query_tokens)
        ...
        # Format output
        if not sorted_docs:
            result = [f"No product information found for: '{query}'"]
        else:
        ...
        logger.info(f"Retrieved {len(result)} results in {processing_time:.2f}s for query: '{query}'")
    ...
    async def batch_retrieve_documents(self, queries: List[str]) -> List[List[str]]:
        ...
        if not self._is_initialized:
            return [["Error: Product retriever is not initialized."] for _ in queries]
        ...
        for query in queries:
            task = asyncio.create_task(
                asyncio.to_thread(self.retrieve_documents, query)
            )
            tasks.append(task)
        ...
        try:
            ...
            return [[f"Batch processing error: {str(e)}"] for _ in queries]
    ...
    def clear_caches(self):
        ...
        logger.info("Clearing product retriever caches...")
        ...
        logger.info("product retriever caches cleared.")
    ...
    def cleanup(self):
        ...
        logger.info("Cleaning up product retriever resources...")
        self._release_gpu_resources()
    ...
    def __getstate__(self):
        ...
        logger.debug("productRetrieverTool prepared for pickling")
        return state
    ...
    def __setstate__(self, state):
        ...
        self._gpu_components_active = False
        logger.debug("productRetrieverTool unpickled, will reinitialize components when needed")
...
if __name__ == '__main__':
    async def main():
        # --- Setup ---
        DATA_DIR = Path("./product_data_test")
        DATA_DIR.mkdir(exist_ok=True)
        
        logger.info(f"Using data directory: {DATA_DIR.resolve()}")
        
        # Create some test files
        with open(DATA_DIR / "genemap_adult.txt", "w", encoding="utf-8") as f:
            f.write("Gói GeneMap Adult là sản phẩm giải mã gen toàn diện cho người lớn. Giá: 5.000.000 VNĐ.")
        
        with open(DATA_DIR / "services.csv", "w", encoding="utf-8") as f:
            f.write("service_name,description,price\n")
            f.write("Tư vấn di truyền,Tư vấn 1-1 với chuyên gia,1.000.000\n")

        # --- Initialization ---
        product_tool = ProductRetrieverTool(watch_directory=str(DATA_DIR))

        # --- Test Retrieval ---
        query = "giá gói genemap adult"
        print(f"\n--- Testing query: '{query}' ---")
        results = await product_tool._arun(query)
        print(results)
        
        # --- Test Auto-Update ---
        print("\n--- Testing auto-update. Adding a new file... ---")
        time.sleep(2) # Wait for observer to be ready
        with open(DATA_DIR / "new_product.txt", "w", encoding="utf-8") as f:
            f.write("Sản phẩm mới: GeneMap Kid dành cho trẻ em. Giúp phát hiện sớm các nguy cơ tiềm ẩn.")
        
        print("New file created. Waiting for the watcher to process...")
        time.sleep(5) # Give the watcher time to detect and process the file

        # --- Test Retrieval Again ---
        query2 = "thông tin về genemap kid"
        print(f"\n--- Testing query after update: '{query2}' ---")
        results2 = await product_tool._arun(query2)
        print(results2)

        # --- Test file modification ---
        print("\n--- Testing file modification... ---")
        time.sleep(2)
        with open(DATA_DIR / "genemap_adult.txt", "a", encoding="utf-8") as f:
            f.write("\nCập nhật: Bao gồm thêm 20 chỉ số mới về dinh dưỡng.")
        
        print("File modified. Waiting for the watcher to process...")
        time.sleep(5)
        
        query3 = "chỉ số dinh dưỡng trong genemap adult"
        print(f"\n--- Testing query after modification: '{query3}' ---")
        results3 = await product_tool._arun(query3)
        print(results3)
        
        # --- Cleanup ---
        product_tool.stop_document_watcher()
        # import shutil
