...
class DocumentWatcher(FileSystemEventHandler):
    ...
    def __init__(self, retriever_tool: 'CompanyRetrieverMCPClient'):
...
class CompanyRetrieverInput(BaseModel):
    """Input schema for company document retrieval."""
    query: str = PydanticField(description="Search query for finding relevant company documents")
...
class CompanyRetrieverMCPClient(BaseAgentTool):
    """Tool to retrieve company documents from a remote vector database via MCP server."""
    name: str = "company_retriever_mcp"
    description: str = "Retrieves company documents from remote vector database and manages document uploads"
    args_schema: type[BaseModel] = CompanyRetrieverInput
    ...
    default_collection: str = PydanticField(default="company_docs", description="Default collection name")
    ...
    def __init__(self, 
                 mcp_server_url: str,
                 watch_directory: str,
                 default_collection: str = "company_docs",
                 **kwargs):
        """
    ...
    async def _initialize_all(self):
        ...
        logger.info(f"Initializing CompanyRetrieverMCPClient...")
        ...
        self._thread_pool = ThreadPoolExecutor(
            max_workers=4,
            thread_name_prefix="company_mcp_client"
        )
        ...
        logger.info("CompanyRetrieverMCPClient initialized successfully")
    ...
    async def _arun(self, query: str, collection_name: str = None, max_results: int = 5) -> str:
        ...
        if not self._is_initialized:
            return "Error: Company retriever client is not initialized."
    ...
    def cleanup(self):
        """Clean up resources."""
        logger.info("Cleaning up CompanyRetrieverMCPClient resources...")
...
def create_company_retriever_client(mcp_server_url: str, 
                                   watch_directory: str,
                                   collection_name: str = "company_docs") -> CompanyRetrieverMCPClient:
    return CompanyRetrieverMCPClient(
        mcp_server_url=mcp_server_url,
...
if __name__ == "__main__":
    async def main():
        ...
        WATCH_DIR = "app/agents/retrievers/storages/companies"
        COLLECTION = "company_knowledge"
        
        # Create the client
        client = create_company_retriever_client(
        ...
        # Test manual upload
        # test_texts = [
        #     "Company policy: All employees must follow safety guidelines.",
        #     "HR policy: Vacation requests must be submitted 2 weeks in advance.",
        # ]
        # upload_result = await client.upload_text_documents(test_texts, COLLECTION)
        # print(f"Upload result: {upload_result}")
        
        # Test query again
        result2 = await client.arun("Giam doc cong ty")
