...
class EmployeeDocumentWatcher(FileSystemEventHandler):
    """File system watcher for automatic employee document loading"""
    ...
    def _is_customer_file(self, file_path: str) -> bool:
        ...
        return (f"employee_{self.retriever.employee_id}" in filename or
                filename.startswith(f"{self.retriever.employee_id}_"))
...
class RetrievedEmployeeDocument(BaseModel):
    ...
    employee_id : str = Field(..., description="Employee ID for specific data retrieval")
    ...
    def calculate_employee_relevance(self, query: str, employee_id: str = "") -> float:
        """Calculate relevance score based on employee-specific query"""
...
class EmployeeRetrieverTool(BaseAgentTool):
    ...
    employee_docs: Dict[str, EmployeeData] = Field(default_factory=dict, description="Employee-specific documents and traits")
    employees_storage_path: str = Field(default="", description="Path to employee storage directory")
   ...
   # New fields for enhanced functionality
    ...
    def __init__(self, employee_id: str, use_bm25: bool = True, auto_scan_enabled: bool = True, watch_directory: str = None, **kwargs):
        # Set up employee-specific collection name
        safe_employee_id = employee_id.replace("-", "_").replace(" ", "_")
        collection_name = f"employee_{safe_employee_id}_data"

        # Set employees storage path and watch directory
        employees_storage_path = os.path.join(os.path.dirname(__file__), 'storages', 'employees')
        watch_directory = watch_directory or employees_storage_path  # Watch the employees directory

        # Initialize tool with employee-specific name and description
        name = f"employee_retriever_{safe_employee_id}"
        description = f"Dynamic employee retrieval tool for medical information and health data for employee {employee_id} with multi-format support"
        ...
        super().__init__(
            ...
            use_bm25=use_bm25,
            employees_storage_path=employees_storage_path,
        ...
        # Initialize components and load employee data
    ...
    def _initialize_components(self):
        ...
        try:
            ...
            logger.info(f"ChromaDB client initialized for employee {self.employee_id}")
            ...
            if self.use_bm25:
                try:
                    ...
                    if all_docs and 'documents' in all_docs and all_docs['documents']:
                        ...
                        logger.info(f"BM25 retriever initialized with {len(documents)} employee documents")
                    else:
                        logger.warning("No employee documents available for BM25 indexing")
            ...
            logger.info(f"Employee retriever initialized successfully for employee {self.employee_id}")

        except Exception as e:
            logger.error(f"Failed to initialize employee retriever: {e}")
            self.is_initialized = False
            self.vector_store = None
            self.bm25_retriever = None

    def _load_employee_data(self):
        """Load employee-specific data from the appropriate employee file"""
        if not self.is_initialized:
            logger.warning("Cannot load employee data: Retriever not initialized")
            return
        # check if not have documents in collection
        if self.vector_store.get().get('documents', []) and self.vector_store.get().get('metadatas', []):
            logger.info(f"Employee {self.employee_id} already has documents in the vector store")
            return
        
        
        try:
            logger.info(f"Checking existing documents for employee {self.employee_id}")
            self._parse_employee_docs_from_vectorstore()
        except Exception as e:
            logger.warning(f"Error checking existing documents: {e}")

        # Try to find employee-specific files
        employee_files = self._find_employee_files()

        if not employee_files:
            logger.warning(f"No employee files found for employee {self.employee_id}")
            return
        else: 
            logger.info(f"Found {len(employee_files)} employee files for employee {self.employee_id}")

        # Load documents from employee files
        for file_path in employee_files:
            try:
                self.load_new_document(file_path)
                logger.info(f"Loaded employee file: {file_path}")
            except Exception as e:
                logger.error(f"Failed to load employee file {file_path}: {e}")

    def _find_employee_files(self) -> List[str]:
        """Find all files belonging to this employee"""
        employee_files = []

        if not os.path.exists(self.employees_storage_path):
            logger.warning(f"Employee storage path does not exist: {self.employees_storage_path}")
            return employee_files

        for filename in os.listdir(self.employees_storage_path):
            file_path = os.path.join(self.employees_storage_path, filename)

            # Check if file belongs to this employee
            if (f"employee_{self.employee_id}" in filename or 
                filename.startswith(f"employee_{self.employee_id}_")):
    ...
    def _parse_employee_docs_from_vectorstore(self):
        try:
            all_docs = self.vector_store.get()
            ...
            for i, doc_text in enumerate(documents):
                ...
                if data_id not in self.employee_docs:
                    docs = EmployeeData(
                    ...
                    self.employee_docs[data_id] = docs

            logger.info(f"Loaded {len(self.employee_docs)} employee data items from vector store for employee {self.employee_id}")

        except Exception as e:
            logger.warning(f"Error parsing employee data from vector store: {e}")
    ...
    def load_new_document(self, file_path: str):
        ...
        try:
            ...
            # Parse employee documents to update the local registry
            self._parse_employee_docs_from_vectorstore()
    ...
    def scan_for_new_documents(self):
        ...
        for root, dirs, files in os.walk(self.watch_directory):
            for file in files:
                ...
                # Check if file belongs to this employee
                if (f"employee_{self.employee_id}" in file or 
                    file.startswith(f"{self.employee_id}_")):
                    logger.debug(f"File belongs to employee {self.employee_id}: {file}")
                    # Check if it's a supported format
        ...
        try:
            ...
            if os.path.exists(self.watch_directory):
                for filename in os.listdir(self.watch_directory):
                    if (f"employee_{self.employee_id}" in filename or
                        filename.startswith(f"{self.employee_id}_")):
                        if any(filename.endswith(ext) for ext in self.supported_formats):
                            total_files += 1
            ...
            status = {
                ...
                'collection_name': self.collection_name,
                'storage_path': self.employees_storage_path,
                'watch_directory': self.watch_directory,
                'total_documents': doc_count,
                'total_contents': len(self.employee_docs),
                'files_in_registry': loaded_files,
                'total_employee_files': total_files,
            ...
            return {'error': str(e), 'employee_id': self.employee_id}

    def get_employee_files(self, file_filter: str = "") -> Dict[str, List[str]]:
        """Get list of employee files, optionally filtered"""
        files_by_type = {}
    ...
    def _run(self, query: str, k: int = 5, search_type: str = "hybrid") -> str:
        """Execute the employee retrieval with dynamic reranking"""
        if not self.is_initialized:
            return f"❌ Truy van nhan vien {self.employee_id} chua duoc khoi tao"

        if not self.vector_store:
            return f"❌ Vector store khong co cho nhan vien {self.employee_id}"
        ...
        try:
            ...
            try:
                ...
                for doc, score in vector_docs:
                    retrieved_doc = RetrievedEmployeeDocument(
                        ...
                        data_type=doc.metadata.get('data_type', 'employee_docs'),
            ...
            if not retrieved_docs:
                return f"ℹ️ Khong tim thay thong tin nao cua nhan vien {self.employee_id} voi truy van: '{query}'"

            # Calculate employee-specific relevance scores
            for doc in retrieved_docs:
                doc.calculate_employee_relevance(query, self.employee_id)
            ...
            # Format response with employee context
            response_parts = [f"🏥 **E {self.employee_id} Information**\n"]
            ...
            response_parts.append(f"📊 **Summary:** Found {len(top_docs)} relevant results from {len(self.employee_docs)} employee documents")

            return "\n".join(response_parts)
            
        except Exception as e:
            logger.error(f"Error during employee retrieval: {e}")
            return f"❌ Error retrieving medical information for employee {self.employee_id}: {str(e)}"

    async def _arun(self, query: str, k: int = 5, search_type: str = "hybrid") -> str:
        """Asynchronously execute the employee retrieval with dynamic reranking"""
    ...
    def get_employee_medical_profile(self) -> Dict[str, Any]:
        """Get a summary of the employee's medical profile"""
        if not self.is_initialized:
        ...
        profile = {
            ...
            "total_data_items": len(self.employee_docs),
            "data_summary": []
        }

        for data_id, data in self.employee_docs.items():
    ...
    def search_customer_conditions(self, condition_query: str) -> List[Dict[str, Any]]:
        ...
        results = []
        for doc in self.employee_docs.values():
    ...
    def cleanup(self):
        ...
        try:
            ...
            self.employee_docs.clear()
            self.document_registry.clear()

            logger.info(f"Employee retriever cleanup completed for employee {self.employee_id}")
...
if __name__ == "__main__":
    # Example usage
    retriever = EmployeeRetrieverTool(employee_id="12345", use_bm25=True, auto_scan_enabled=True)
