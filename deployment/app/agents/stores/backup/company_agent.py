...
class CompanyAgent(Agent):
    def __init__(self, llm: BaseChatModel, default_tools: List[BaseAgentTool], vectorstore: Optional[Chroma] = None):
        ...
        system_msg = agent_config['company_agent']['description']
        super().__init__(llm, "CompanyAgent", system_msg, default_tools, vectorstore)
        
        self.retriever = CompanyRetrieverTool(collection_name="company_docs", watch_directory='app/agents/retrievers/storages/company')
...
if __name__ == "__main__":
    # Example usage of the CompanyAgent
    llm = llm_instance()
    default_tools = []  # Add any default tools if needed
    vectorstore = None  # Initialize your vectorstore if required

    company_agent = CompanyAgent(llm, default_tools, vectorstore)
    
    # Example state for execution
    state = GraphState(
        original_query="What is the company's mission?",
        chat_history=[("What is our mission?", "Our mission is to...")],
        intents="retrieval",
        # rewritten_query="Company mission"
    )
    
    result = company_agent.execute(state)
    print(result)