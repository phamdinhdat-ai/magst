...
class DrugAgent(Agent):
    def __init__(self, llm: BaseChatModel, default_tools: List[BaseAgentTool], vectorstore: Optional[Chroma] = None):
        ...
        system_msg = agent_config['drug_agent']['description']
        super().__init__(llm, "DrugAgent", system_msg, default_tools, vectorstore)

        self.retriever = DrugRetrieverTool(collection_name="drug_docs", watch_directory='app/agents/retrievers/storages/drugs')
...
if __name__ == "__main__":
    llm = llm_instance
    default_tools = [DrugRetrieverTool()]
    drug_agent = DrugAgent(llm, default_tools)
    
    state = GraphState(
        original_query="What is the drug's mechanism of action?",
        chat_history=[("What is the drug?", "The drug is used for...")],
        customer_id="789122254025",
        intents=["retrieve"],
        # rewritten_query="Drug mechanism of action"
    )
    result = drug_agent.execute(state)
    print(result)