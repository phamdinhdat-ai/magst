...
# Import the HistoryCache
from app.agents.stores.history_cache import HistoryCache, get_history_cache
...
class GuestWorkflow:
    ...
    Now includes session-based chat history management.
    """
    def __init__(self, max_iterations: int = 2): # Reduced for faster guest responses
        ...
        # Initialize history cache for session management
        self.history_cache = get_history_cache(
            max_sessions=1000,  # Support up to 1000 concurrent guest sessions
            session_ttl_hours=24,  # Sessions expire after 24 hours
            max_messages_per_session=50,  # Keep last 50 messages per session
            enable_persistence=True,  # Enable disk persistence for reliability
            persistence_path="app/cache/guest_history"
        )
        
        logger.add(Path("app/logs/log_workflows/guest_workflow.log"), rotation="10 MB", level="DEBUG", backtrace=True, diagnose=True)
        logger.info("Guest Workflow initialized with caching and history management support.")
    ...
    def _initialize_agents(self) -> Dict[str, Any]:
        ...
        *** KHÔNG BAO GỒM CustomerAgent và EmployeeAgent. ***
        ...
        return {
            ...
            "rewriter": RewriterAgent(llm=llm_reasoning),
            "reflection": ReflectionAgent(llm=llm_reasoning, default_tool_names=[]),
            "supervisor": SupervisorAgent(llm=llm),
            "question_generator": QuestionGeneratorAgent(llm=llm_reasoning),

            # Các agent chuyên môn được phép cho khách
            "CompanyAgent": CompanyAgent(llm=llm_reasoning, default_tool_names=["company_retriever_tool"]),
            "ProductAgent": ProductAgent(llm=llm_reasoning, default_tool_names=["product_retriever_tool"]),
            "MedicalAgent": MedicalAgent(llm=llm_reasoning, default_tool_names=["medical_retriever_tool"]),
            "DrugAgent": DrugAgent(llm=llm_reasoning, default_tool_names=["drug_retriever_tool"]),
            "GeneticAgent": GeneticAgent(llm=llm_reasoning, default_tool_names=["genetic_retriever_tool"]),
            "NaiveAgent": NaiveAgent(llm=llm, default_tool_names=[]),
            "GuestAgent": GuestAgent(llm=llm_reasoning, default_tool_names=["company_retriever_tool"]),
            "SummaryAgent": SummaryAgent(llm=llm_reasoning),
        }
    ...
    async def create_guest_session(
        ...
        """
        Create a new guest session with history tracking.
        ...
        logger.info(f"Created guest session: {session_id} for guest: {guest_id}")
    ...
    async def get_session_history(
        ...
        Returns:
            List of chat messages formatted for workflow
        """
        return await self.history_cache.get_chat_history(
            session_id, 
            limit=limit, 
            format_for_workflow=True
        )

    async def add_conversation_to_history(
        self,
        session_id: str,
        user_query: str,
        ...
        """
        # Add user message metadata
        user_metadata = {"query_type": "guest", "timestamp": datetime.utcnow().isoformat()}
        if metadata:
            user_metadata.update(metadata.get("user", {}))
        ...
        if metadata:
            assistant_metadata.update(metadata.get("assistant", {}))
    ...
    async def _prepare_initial_state(
        ...
        """
        Prepare initial state with chat history from the session.
        """
        # Get chat history for this session
        chat_history = await self.get_session_history(session_id, limit=20)  # Last 20 messages
        ...
        initial_state = AgentState(
            ...
            iteration_count=0,
            chat_history=chat_history,  # Use session-specific history
            user_role="guest", 
            guest_id=guest_id,
        ...
        logger.info(f"Prepared initial state with {len(chat_history)} historical messages for session {session_id}")
    ...
    def _should_summarize(self, state: AgentState) -> str:
        ...
        chat_history_threshold = getattr(self, 'chat_history_threshold', 30)  # Default threshold
        
        if history_len > chat_history_threshold:
    ...
    async def arun_streaming(
        ...
        Now includes session-based history management.
        """
        ...
        if not session_id:
            ...
            guest_id, session_id = await self.create_guest_session(guest_id)
            config["configurable"]["thread_id"] = session_id
        elif not guest_id:
            # Try to get guest_id from existing session
            session = await self.history_cache.get_session(session_id)
            if session:
                guest_id = session.guest_id
            else:
                guest_id = f"guest_{uuid.uuid4()}"
        
        # Override chat_history with session history if not provided
        if not chat_history:
            chat_history = await self.get_session_history(session_id, limit=20)
        
        # Check cache first for faster responses (include session_id in cache key)
        if self.cache_manager.is_active():
            try:
                # Create session-aware cache key
                cache_key = f"{self.cache_manager.create_cache_key(query, chat_history)}_{session_id}"
                logger.debug(f"Checking cache with session-aware key: {cache_key}")
                ...
                if cached_result:
                    ...
                    if agent_response:
                        ...
                        response_chunk = ''
                        for word in agent_response_words:
                            response_chunk += word + " "
                            yield {
                                "event": "answer_chunk",
                                "data": response_chunk,
                    ...
                    await self.add_conversation_to_history(
                        session_id=session_id,
                        user_query=query,
        ...
        # Prepare initial state with session history
        initial_state = await self._prepare_initial_state(query, session_id, guest_id, config)
                ...
                try:
                    ...
                    if kind == "on_chain_stream":
                        ...
                        if isinstance(chunk, dict) and "agent_response" in chunk:
                            ...
                            final_response = response  # Store for history
                            yield {
                                "event": "answer_chunk",
                                "data": response,
                                "metadata": {
                        ...
                        if node_name == "final_processing":
                            try:
                                ...
                                final_response = final_state.get("agent_response", final_response)
                                
                                final_result_data = {
                                    "suggested_questions": final_state.get("suggested_questions", []),
                                    "full_final_answer": final_response,
                                ...
                                await self.add_conversation_to_history(
                                    ...
                                    metadata={
                                        "agents_used": final_result_data["agents_used"],
                                        "processing_time": final_result_data["processing_time"]
                                    }
                                )
    ...
    async def arun_simple(
        ...
        async for event in self.arun_streaming(query, config, guest_id, session_id, chat_history):
            if event.get("event") == "answer_chunk":
                full_answer = event["data"]  # Keep updating with latest chunk
    ...
    async def process_fast_mode(
        ...
        """
        Fast processing mode for guest queries.
        Now includes session-based history management.
        """
        ...
        # Override chat_history with session history if not provided
        if not chat_history:
            chat_history = await self.get_session_history(session_id, limit=10)  # Fewer messages for fast mode
        ...
        if self.cache_manager.is_active():
            ...
            if cached_result:
                ...
                cached_result["guest_id"] = guest_id
        ...
        try:
            ...
            initial_state = await self._prepare_initial_state(query, session_id, guest_id, {})
            initial_state["max_iterations"] = 1  # Reduced for speed
            initial_state["next_agent"] = "entry"  # Start with entry agent
            ...
            try:
                ...
                result = await asyncio.wait_for(
                    ...
                    timeout=40  # 40 second timeout (optimized for speed)
                )
                ...
                return await self._generate_timeout_response(query, session_id, guest_id)
    ...
    async def _generate_timeout_response(self, query: str, session_id: str, guest_id: str) -> Dict[str, Any]:
        """Generate a response when processing times out"""
        response = self._generate_contextual_response(query)
        
        # Still add to history even if timed out
        await self.add_conversation_to_history(
            session_id=session_id,
            user_query=query,
            assistant_response=response,
            metadata={"processing_mode": "timeout_fallback"}
        )
    ...
    async def delete_guest_session(self, session_id: str) -> bool:
        """Delete a guest session and its history."""
        return await self.history_cache.delete_session(session_id)

    async def clear_guest_history(self, guest_id: str) -> int:
        """Clear all sessions for a guest."""
        return await self.history_cache.clear_guest_sessions(guest_id)

    async def get_workflow_stats(self) -> Dict[str, Any]:
        """Get workflow and cache statistics."""
        cache_stats = await self.history_cache.get_session_stats()
        
        workflow_stats = {
            "workflow_type": "guest",
            "max_iterations": self.max_iterations,
            "agents_available": list(self.agents.keys()),
            "cache_manager_active": self.cache_manager.is_active() if self.cache_manager else False
        }
        
        return {
            "workflow": workflow_stats,
            "history_cache": cache_stats
        }
...
async def create_guest_workflow_session(
    guest_id: Optional[str] = None,
    workflow: Optional[GuestWorkflow] = None
) -> tuple:
    """Create a new workflow session for guest."""
    if not workflow:
        workflow = GuestWorkflow()
    
    guest_id, session_id = await workflow.create_guest_session(guest_id)
    logger.info(f"Created guest workflow session: {session_id} for guest {guest_id}")
    return guest_id, session_id


async def get_guest_session_history(
    session_id: str,
    workflow: Optional[GuestWorkflow] = None,
    limit: Optional[int] = None
) -> List[Dict[str, Any]]:
    ...
    return await workflow.get_session_history(session_id, limit)
...
if __name__ == "__main__":
    async def main():
        ...
        logger.info("====== TESTING GUEST WORKFLOW WITH HISTORY CACHE ======")
        
        workflow_manager = GuestWorkflow()
        
        # Test session creation and history management
        logger.info("-" * 80)
        logger.info("🚀 TESTING SESSION CREATION AND HISTORY MANAGEMENT")
        
        # Create a new session
        guest_id, session_id = await workflow_manager.create_guest_session()
        logger.info(f"Created session: {session_id} for guest: {guest_id}")
        
        # Test first query
        query1 = "What is GeneStory?"
        logger.info(f"Processing first query: {query1}")
        result1 = await workflow_manager.arun_simple(query1, guest_id, session_id)
        logger.info(f"First result: {result1.get('agent_response', '')[:100]}...")
        
        # Test second query with history
        query2 = "How much do your services cost?"
        logger.info(f"Processing second query: {query2}")
        result2 = await workflow_manager.arun_simple(query2, guest_id, session_id)
        logger.info(f"Second result: {result2.get('agent_response', '')[:100]}...")
        
        # Check session history
        history = await workflow_manager.get_session_history(session_id)
        logger.info(f"Session history contains {len(history)} messages")
        for i, msg in enumerate(history):
            logger.info(f"  Message {i+1}: {msg['role']} - {msg['content'][:50]}...")
        
        # Test session info
        session_info = await workflow_manager.get_session_info(session_id)
