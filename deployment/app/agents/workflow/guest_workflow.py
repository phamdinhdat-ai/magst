...
from app.agents.mcp_session import get_session
class GuestWorkflow:
    """
    ...
    def _initialize_agents(self) -> Dict[str, Any]:
        ...
        *** DOES NOT INCLUDE CustomerAgent or EmployeeAgent. ***
        ...
        return {
            ...
            "NaiveAgent": NaiveAgent(llm=llm_instance, default_tool_names=["searchweb_tool"]),

            # General Purpose Agents
            "FallbackAgent": NaiveAgent(llm=llm_instance),
            "DirectAnswerAgent": NaiveAgent(llm=llm_instance),

            # All Specialist Agents available to guests (public data only)
            "CompanyAgent": CompanyAgent(llm=llm_instance, default_tool_names=["company_retriever_mcp_tool"]),
            "ProductAgent": ProductAgent(llm=llm_instance, default_tool_names=["product_retriever_mcp_tool"]),
            "MedicalAgent": MedicalAgent(llm=llm_instance, default_tool_names=["medical_retriever_mcp_tool"]),
            "DrugAgent": DrugAgent(llm=llm_instance, default_tool_names=["drug_retriever_mcp_tool"]),
            "GeneticAgent": GeneticAgent(llm=llm_instance, default_tool_names=["genetic_retriever_mcp_tool"]),
        }
    ...
    async def _run_specialist_node(self, state: AgentState) -> AgentState:
        ...
        allowed_guest_agents = [
            ...
            "VisualAgent", "DirectAnswerAgent", "NaiveAgent", "MedicalAgent", "DrugAgent"
        ]
    ...
    async def _final_answer_node(self, state: AgentState) -> AgentState:
        ...
        # Use aexecute() for LangGraph integration - it will emit on_chat_model_stream events
        # that LangGraph can detect and forward to the workflow's astream_events
        return await agent.aexecute(state)
    ...
    async def arun_streaming(
        ...
        try:
            ...
            async for event in self.graph.astream_events(initial_state, config=config, version="v1"):
                try:
                    # Handle cases where event might not be a dictionary (e.g., Pydantic models)
                    if not isinstance(event, dict):
                        logger.debug(f"Skipping non-dict event: {type(event).__name__}")
                        continue
                    
                    # Additional safety check for get method
                    if not hasattr(event, 'get'):
                        logger.debug(f"Skipping event without get method: {type(event).__name__}")
                        continue
                    ...
                    if kind == "on_chain_start":
                        ...
                        # Safely extract node data, avoiding non-serializable objects
                        ...
                        for key, value in event_data.items():
                            try:
                                ...
                                import json
                                json.dumps(value)
                                safe_node_data[key] = value
                            except (TypeError, ValueError):
                                # Skip non-serializable values like AIMessage objects
                                logger.debug(f"Skipping non-serializable node data key: {key} (type: {type(value).__name__})")
                                continue
                        
                        # Send node_start event with node_name and safe data
                        yield {
                            "event": "node_start",
                            "data": {
                                "node": node_name,
                    ...
                    # Handle streaming chunks from the LLM
                        ...
                        # Process when we get agent_response dict, but ONLY from final_answer node
                        if isinstance(chunk, dict) and "agent_response" in chunk and node_name == "final_answer":
                            response = chunk.get("agent_response", "")
                            if response:
                                # Only send response_checkpoint events for final_answer node
                                logger.debug(f"Streaming final answer checkpoint response: {response[:50]}...")
                                yield {
                        ...
                        # Only stream direct text chunks for the final_answer node
                        elif isinstance(chunk, str) and chunk.strip() and node_name == "final_answer":
                            logger.debug(f"Streaming raw text chunk from final_answer: {chunk[:50]}...")
                            yield {
                                "event": "token_chunk",
                                "data": chunk,
                                "metadata": {
                    ...
                    # Handle direct LLM streaming events - these are the raw token-by-token chunks
                        ...
                        if chunk_text and hasattr(chunk_text, 'content'):
                            ...
                            # ONLY stream chunks from final_answer node
                            # logger.debug(f"Streaming LLM chunk from {node_name}: {chunk_content[:50]}...")
                            if chunk_content and node_name == "final_answer":
                                ...
                                # Other nodes' chunks will be ignored completely
                    ...
                    # Handle node completion events
                        ...
                        if node_name == "triage_node":
                            ...
                            # Handle case where output might be a Pydantic model
                            if hasattr(output_data, 'is_toxic'):
                                is_toxic = output_data.is_toxic
                            ...
                            if is_toxic:
                                # Handle getting toxicity reason and safety response
                                if hasattr(output_data, 'toxicity_reason'):
                                    toxicity_reason = output_data.toxicity_reason or "Content policy violation"
                                    safety_response = getattr(output_data, 'safety_response', "I cannot assist with that request.")
                                elif isinstance(output_data, dict):
                                    ...
                                    toxicity_reason = "Content policy violation"
                                    safety_response = "I cannot assist with that request."
                        ...
                        if node_name == "question_generator":
                            try:
                                ...
                                # Handle case where output might be a Pydantic model
                                if hasattr(output_data, 'suggested_questions'):
                                    suggested_questions = output_data.suggested_questions or []
                                elif isinstance(output_data, dict):
                                    suggested_questions = output_data.get("suggested_questions", [])
                                else:
                                    suggested_questions = []
                                ...
                                logger.error(f"Error processing question generator output: {str(question_error)}")
                        ...
                        # Special handling for the final answer node
                            ...
                            try:
                                ...
                                # Handle case where final_state might be a Pydantic model
                                if hasattr(final_state, 'agent_response'):
                                    logger.debug("Final state is a Pydantic model, extracting agent_response")  
                                    logger.debug(f"Final agent response: {final_state.agent_response[:50]}...")
                                    final_answer = final_state.agent_response or ""
                                ...
                                if hasattr(final_state, 'suggested_questions'):
                                    suggested_questions = final_state.suggested_questions or []
                                elif isinstance(final_state, dict):
                                    suggested_questions = final_state.get("suggested_questions", [])
                                else:
                                    suggested_questions = []
                                
                                if hasattr(final_state, 'agent_thinks'):
                                    agent_thinks = final_state.agent_thinks or {}
                                elif isinstance(final_state, dict):
                                ...
                                final_result_data = {
                                    ...
                                    "agents_used": list(agent_thinks.keys()),
                        ...
                        # Special handling for the question generator node
                            ...
                            try:
                                ...
                                # Handle case where question_state might be a Pydantic model or AIMessageChunk
                                if hasattr(question_state, 'suggested_questions'):
                                    suggested_questions = question_state.suggested_questions or []
                                elif isinstance(question_state, dict):
    ...
    async def astreaming_workflow(self, query: str, config: Dict = None, 
        ...
        """
        logger.info(f"Starting direct graph streaming for guest query: {query[:100]}...")
        ...
        try:
            ...
            async for event in self.graph.astream_events(initial_state, config=config, version="v1"):
                ...
                # logger.debug(f"Received event: {kind} from node: {event}")
                    ...
                    if node_name == "final_answer":
                        ...
                        if kind == "on_chat_model_stream":
                            ...
                            if chunk and hasattr(chunk, "content"):
                                token = chunk.content
                                # logger.info(f"Streaming chunk: {token[:50]}...")
                                yield {
                                "event": "token_chunk",
                                "data": token,
                                "node": "final_answer"
                            ...
                            final_answer += token
                            await asyncio.sleep(0.03)  # Slight delay to simulate streaming
                        elif kind == "on_chain_stream":
                            chunk = event.get("data", {}).get("chunk")
                            if chunk and hasattr(chunk, "content") and isinstance(chunk.content, str):
                                token = chunk.content
                                # logger.debug(f"Streaming chunk on stream: {token[:50]}...")
...
if __name__ == '__main__':
    async def test_guest_workflow():
        """A comprehensive test suite to validate the secure and optimized GuestWorkflow."""
        print("\n" + "="*80)
        ...
        try:
            ...
            print("✅ GuestWorkflow initialized successfully")
        except Exception as e:
            print(f"❌ Failed to initialize GuestWorkflow: {e}")
            return
        ...
        test_cases = [
            {
                ...
                "query": "Hello",
                "history": [],
                "expected_types": ["stream_start", "final_stream_chunk"]
            },
            {
                "name": "✅ SPECIALIST PATH (Public Company Info)",
                "query": "What is your company's mission statement?",
                "history": [],
                "expected_agent": "CompanyAgent"
            },
            {
                "name": "🔒 SECURITY TEST (Personal Data Access)",
                "query": "What was my last order?",
                "history": [],
                "security_test": True
            },
            {
                "name": "🔒 SECURITY TEST (Employee Data Access)",
            ...
            {
                "name": "🔧 ERROR HANDLING (Long Query)",
        ...
        for i, case in enumerate(test_cases):
            ...
            config = {"configurable": {"thread_id": f"test_{uuid.uuid4()}"}}
            ...
            try:
                async for event in workflow.arun_streaming(
                    ...
                    if event_type == 'node_start':
                        ...
                        content_filtered = True
                        print(f"\n  🚫 \033[91mCONTENT FILTERED:\033[0m {event.get('data', {}).get('reason', 'Unknown')}")
                        print(f"  🛡️  \033[93mSafety Response:\033[0m {event.get('data', {}).get('safety_response', 'No response')}")
                        full_response = event.get('data', {}).get('safety_response', '')

                    elif event_type == 'toxicity_detected':
                        is_toxic = True
                        content_filtered = True
                        print(f"\n  ☢️ \033[91mTOXICITY DETECTED:\033[0m {event.get('data', {}).get('reason', 'Unknown')}")

                    elif event_type == 'final_stream_chunk':
                        new_part = str(event.get('data', ''))
                        ...
                        if not content_filtered:  # Only update if not filtered
                            response_data = str(event.get('data', ''))
                            new_part = response_data.replace(full_response, "", 1) if full_response else response_data
                            if not full_response and new_part:
                                print("\n  \033[92mBOT:\033[0m ", end="")
                            if new_part:
                                print(f"\033[92m{new_part}\033[0m", end="", flush=True)
                            full_response = response_data
                    
                    elif event_type in ['final_result', 'final_complete']:
                        final_data = event.get('data', {})
                    
                    elif event_type == 'error':
                        error_data = event.get('data', {})
                        print(f"\n  ❌ \033[91mERROR:\033[0m {error_data.get('error', 'Unknown error')}")
            ...
            print(f"  - Path Taken: {' -> '.join(nodes_visited) if nodes_visited else 'No nodes visited'}")
            print(f"  - Events: {', '.join(set(events_collected))}")
            
            # Test validation
            test_passed = True
            
            # Validate toxicity tests
            if case.get('toxicity_test'):
                if content_filtered or is_toxic:
            ...
            elif case.get('security_test'):
                prohibited_terms = ["customer", "employee", "personal", "internal", "admin", "order", "vacation"]
                found_prohibited = [term for term in prohibited_terms if term.lower() in full_response.lower()]
                if not found_prohibited:
                    print("  - \033[92mSecurity Test Result: PASSED. Agent correctly refused to access private data.\033[0m")
                else:
                    print(f"  - \033[91mSecurity Test Result: FAILED. Found prohibited terms: {found_prohibited}\033[0m")
                    test_passed = False
            
            # Validate error handling tests
            elif case.get('error_test'):
                error_events = [e for e in events_collected if e == 'error']
                if error_events or case['query'] == "":
                    print("  - \033[92mError Handling Test: PASSED. Error was properly handled.\033[0m")
                else:
                    print("  - \033[91mError Handling Test: FAILED. Expected error was not handled.\033[0m")
                    test_passed = False
            
            # Validate performance tests
            elif case.get('performance_test'):
                if execution_time < 30.0:  # 30 second threshold for very long queries
                    print(f"  - \033[92mPerformance Test: PASSED. Response time acceptable ({execution_time:.2f}s).\033[0m")
                else:
                    print(f"  - \033[91mPerformance Test: FAILED. Response too slow ({execution_time:.2f}s).\033[0m")
                    test_passed = False
            
            # Validate expected agent routing
            elif case.get('expected_agent'):
                expected_agent = case['expected_agent']
                # Check if the expected agent was mentioned in the final data or if appropriate specialist nodes were visited
                agent_found = (expected_agent.lower() in str(final_data).lower() or 
                             'specialist_agent' in nodes_visited or 
                             expected_agent.replace('Agent', '').lower() in full_response.lower())
                if agent_found:
                    print(f"  - \033[92mAgent Routing Test: PASSED. Expected agent '{expected_agent}' was used.\033[0m")
                else:
                    print(f"  - \033[91mAgent Routing Test: FAILED. Expected agent '{expected_agent}' was not used.\033[0m")
                    test_passed = False
            
            # Validate basic functionality
            else:
                expected_events = case.get('expected_types', [])
                if all(event in events_collected for event in expected_events):
                    print("  - \033[92mBasic Functionality Test: PASSED. All expected events occurred.\033[0m")
                else:
                    missing_events = [e for e in expected_events if e not in events_collected]
                    print(f"  - \033[91mBasic Functionality Test: FAILED. Missing events: {missing_events}\033[0m")
                    test_passed = False
        ...
        if test_results['failed'] == 0:
            print("\n🎉 All tests passed! GuestWorkflow is functioning correctly.")
        else:
            print(f"\n⚠️  {test_results['failed']} test(s) failed. Please review the results above.")
