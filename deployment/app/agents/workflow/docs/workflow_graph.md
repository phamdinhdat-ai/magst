...
This GraphTD diagram visualizes the workflow, highlighting the main execution path, the conditional logic, and the new sentiment-driven re-execution loop.
...
graph TD
    subgraph Legend
        direction LR
        rect_node[Node]
        diamond_node{Conditional<br>Routing}
        circle_node((Start/End))
    end
    ...
    subgraph "Pool of Specialist Agents"
        direction LR
        style Specialist fill:#f9f,stroke:#333,stroke-width:2px
