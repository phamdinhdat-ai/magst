...
from app.agents.guadrails.contexts_guadrails import ContextGuardrail, context_guardrail_node
from app.agents.guadrails.response_guadrails import ResponseGuardrail, response_guardrail_node
...
class CustomerWorkflow:
    ...
    def _initialize_agents(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "sentiment_analysis": sentiment_analysis_agent,  # NEW
    ...
    async def _sentiment_analysis_node(self, state: AgentState) -> AgentState:
        ...
        sentiment_agent = self.agents["sentiment_analysis"]
    ...
    def _build_and_compile_graph(self) -> StateGraph:
        ...
        workflow.add_node("sentiment_analysis", self._sentiment_analysis_node)  # NEW
        ...
        workflow.add_conditional_edges("entry", self._route_after_entry, {
            "rewriter": "rewriter",
            "sentiment_check": "sentiment_analysis"  # NEW path
        })
        ...
        workflow.add_conditional_edges("sentiment_analysis", self._route_after_sentiment_analysis, {
    ...
    async def arun_streaming(self, query: str, config: Dict, customer_id: str = None, user_role: str = "customer", chat_history: list = None) -> AsyncGenerator[Dict[str, Any], None]:
        ...
        try:
            if self.cache_manager.is_active():
                ...
                cache_key = self.cache_manager.create_cache_key(query, chat_history, context=cache_context)
