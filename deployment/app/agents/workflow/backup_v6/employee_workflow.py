        ...
        async for event in self.graph.astream_events(initial_state, config=config, version="v1"):
            ...
            if kind == "on_chain_start":
                ...
                if isinstance(chunk, dict) and "agent_response" in chunk:
                    logger.info(f"Chucking chunk from node: {event.get('name')} - {chunk.get('agent_response', '')[:50]}...")
                    yield {
                        "event": "answer_chunk",
...
def validate_employee_access(employee_role: str, requested_feature: str) -> bool:
    """Validate if employee role has access to requested feature."""
    access_matrix = {
        "employee": ["basic_search", "company_info", "product_info", "employee_data"],
        "senior_employee": ["basic_search", "company_info", "product_info", "employee_data", "advanced_search"],
        "manager": ["basic_search", "company_info", "product_info", "employee_data", "advanced_search", "team_management"],
        "director": ["basic_search", "company_info", "product_info", "employee_data", "advanced_search", "team_management", "strategic_data"],
        "executive": ["basic_search", "company_info", "product_info", "employee_data", "advanced_search", "team_management", "strategic_data", "executive_reports"],
        "admin": ["all_features"]
    }
    
    allowed_features = access_matrix.get(employee_role, [])
...
if __name__ == '__main__':
    async def main():
        ...
        test_queries = [
            "Hello, how are you today?", # Should take the fast path
            "What are the company policies for remote work?", # Should take the specialist path
            "Tell me about employee benefits.", # Should route to EmployeeAgent
            "What are the side effects of Paracetamol?" # Should route to MedicalAgent
        ]
        
        for q in test_queries:
            print(f"\n{'='*20} TESTING EMPLOYEE QUERY: {q} {'='*20}")
            full_response = ""
            async for event in workflow.arun_streaming(q):
                if event['event'] == 'node_start':
                    ...
                    print("\n---")
                    print(f"Suggested Questions: {event['data']['suggested_questions']}")
            print(f"\n{'='*60}\n")
    ...
    async def arun_streaming_authenticated(
        ...
        async for event in self.graph.astream_events(initial_state, config=config, version="v1"):
            ...
            if kind == "on_chain_start":
                ...
                chunk = event.get("data", {}).get("chunk", {})
                if isinstance(chunk, dict) and "agent_response" in chunk:
                    yield {"event": "answer_chunk", "data": chunk.get("agent_response", "")}
